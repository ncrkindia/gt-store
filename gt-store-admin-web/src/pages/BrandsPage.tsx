...
const BrandsPage = () => {
    ...
    const handleExportCSV = () => {
        const headers = ['id', 'name', 'slug', 'imageUrl', 'description'];
        ...
        link.setAttribute('download', `brands_catalog_${new Date().toISOString().split('T')[0]}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        toast.success('Successfully exported filtered brands catalog.');
    };
    ...
    const handleDownloadTemplate = () => {
        const headers = ['id', 'name', 'slug', 'imageUrl', 'description'];
        const rows = [
            ['', 'Apple', 'apple', 'https://example.com/logo-apple.png', 'Premium consumer electronics maker.'],
            ['', 'Samsung', 'samsung', 'https://example.com/logo-samsung.png', 'Global conglomerate and electronic brand.']
        ];
        const csvContent = generateCSV(headers, rows);
        const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        ...
        link.click();
        document.body.removeChild(link);
        toast.success('Downloaded brand import template.');
    };
    ...
    const processCSV = (file: File) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                const idKeyIdx = headers.findIndex(h => ['id', 'brandid', 'brand_id'].includes(h.replace(/[^a-z0-9]/g, '')));
                const nameKeyIdx = headers.findIndex(h => ['name', 'brandname', 'brand_name'].includes(h.replace(/[^a-z0-9]/g, '')));
                const slugKeyIdx = headers.findIndex(h => ['slug'].includes(h.replace(/[^a-z0-9]/g, '')));
                const imageKeyIdx = headers.findIndex(h => ['imageurl', 'image_url', 'logo', 'image'].includes(h.replace(/[^a-z0-9]/g, '')));
                ...
                toast.info(`Successfully parsed ${mappedData.length} records matching Brand target fields.`);
            } catch (err: any) {
    ...
    const runImport = async () => {
        ...
        for (let i = 0; i < parsedRows.length; i++) {
            ...
            try {
                ...
                if (!name) {
                    throw new Error(`Missing mandatory brand field: name.`);
                }
                
                // Check if brand exists in local state array (by ID, slug, or exact name)
                const existing = brands.find(b => 
                    (id && b.id && b.id.toLowerCase() === id.toLowerCase()) ||
                    (slug && b.slug && b.slug.toLowerCase() === slug.toLowerCase()) ||
                    (name && b.name && b.name.toLowerCase() === name.toLowerCase())
                );
                
                const payload = { name, slug, imageUrl, description };
                
                if (existing) {
                    // Update
                    await apiClient.put(`/api/brands/${existing.id}`, payload);
                    setImportProgress(prev => ({
                        ...prev,
                        current: rowNum,
                        success: prev.success + 1,
                        logs: [...prev.logs, { type: 'success', message: `Row ${rowNum}: Successfully updated Brand "${name}" (ID: ${existing.id}).` }]
                    }));
                } else {
                    // Create
                    ...
                    setImportProgress(prev => ({
                        ...
                        current: rowNum,
                        success: prev.success + 1,
                        logs: [...prev.logs, { type: 'success', message: `Row ${rowNum}: Successfully registered new Brand "${name}".` }]
                    }));
                }
            } catch (err: any) {
        ...
        toast.success(`Bulk brand catalog synchronization finished.`);
        fetchBrands();
    };
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-6 mb-6 gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                        <h1 className="text-2xl font-bold bg-gradient-to-r from-indigo-500 to-purple-500 bg-clip-text text-transparent">Brand Management</h1>
                        <p className="text-slate-500 text-xs font-semibold mt-1">
                            Configure manufacturer assets, update identity files, or bulk sync brand catalogues.
                        </p>
                    </div>
                </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                                <h2 className="text-lg font-black text-slate-800 flex items-center gap-2">
                                    <span>Bulk Import Brands catalogue</span>
                                ...
                                <p className="text-xs font-semibold text-slate-400">Upload CSV sheet to batch create or update registered brand configurations.</p>
                            </div>
                        </div>
                                ...
                                <div className="bg-slate-50/50 border border-slate-200/60 p-6 rounded-2xl">
                                    ...
                                    <div className="space-y-3 text-left">
                                        <div className="flex gap-2 text-xs">
                                            ...
                                            <span className="text-slate-500 font-medium">(Required) The brand name.</span>
                                        </div>
                                        <div className="flex gap-2 text-xs">
                                        ...
                                        <div className="flex gap-2 text-xs">
                                            ...
                                            <span className="text-slate-500 font-medium">(Optional) Absolute logo URL.</span>
                                        </div>
                                        <div className="flex gap-2 text-xs">
                                            <span className="font-extrabold text-indigo-600 shrink-0">description</span>
                                            <span className="text-slate-500 font-medium">(Optional) Long details.</span>
                                        </div>
                                    </div>
                            ...
                            <div className="lg:col-span-7 space-y-6">
                                {importProgress.status === 'idle' ? (
                                    <div className="space-y-6">
                                        {validationError && (
                                            <div className="bg-red-50/70 border border-red-100 rounded-2xl p-4 text-xs font-bold text-red-700 flex items-start gap-3">
                                            ...
                                            <div className="border border-slate-200/80 rounded-2xl overflow-hidden shadow-3xs bg-white">
                                                <div className="bg-slate-50/80 px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                                                    <div className="flex flex-col text-left">
                                                        ...
                                                        <span className="text-[10px] text-slate-400 font-semibold mt-0.5">Showing mapped Brand attributes used for operations</span>
                                                    </div>
                        ...
                        <div>
                            {importProgress.status === 'processing' && (
                                <span className="text-xs text-slate-400 font-semibold flex items-center gap-2 animate-pulse">
                                    ...
                                    Syncing brand catalogues. Please do not close this window...
                                </span>
                            )}
                        </div>
                        <div className="flex items-center gap-3">
                            {importProgress.status === 'idle' && (
                                <>
                                    ...
                                    <button 
                                        ...
                                        onClick={runImport}
                                        className="inline-flex items-center gap-2 px-6 py-2.5 bg-indigo-600 peer peer-checked:bg-indigo-750 text-white text-xs font-bold rounded-xl shadow-md transition cursor-pointer disabled:opacity-40 disabled:pointer-events-none"
                                    >
